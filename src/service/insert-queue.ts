...
import { ALLOED_LANGUAGES } from "./allowed-languages";
...
export default class InsertQueue {
	...
	public async add(item: InsertItem): Promise<AddRepositoryQueueResult> {
		...
		if (
			this._queue.find((i) => i.owner === item.owner && i.repo === item.repo)
		) {
		...
		try {
			...
			if (!ALLOED_LANGUAGES.includes(repoLanguage)) {
				return { success: false, error: `Sorry, ${repoLanguage} Language is not supported for analysis` };
			}
		} catch (error) {
			return { success: false, error: "Repository does not exist" };
		}
